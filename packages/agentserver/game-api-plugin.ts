...
const gameAPIRoutes: Route[] = [
  ...
  // Chat message ingestion endpoint (required by Tauri frontend)
  {
    type: 'POST',
    path: '/api/chat',
    name: 'Ingest Chat Message',
    public: true,
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        const { text, roomId, userId } = req.body;

        console.log('[GAME-API] Received chat message:', { text, roomId, userId });

        if (!text || typeof text !== 'string') {
          return res.status(400).json(errorResponse('INVALID_MESSAGE', 'Message text is required'));
        }

        // Use a default room ID if not provided
        const messageRoomId = roomId || 'default-chat-room';
        const messageUserId = userId || 'user';

        // Create the message memory using the correct API
        let messageMemory;
        try {
          // Try different ways to access the memory manager
          if (runtime.messageManager && runtime.messageManager.createMemory) {
            messageMemory = await runtime.messageManager.createMemory({
              ...
              agentId: runtime.agentId,
              embedding: await runtime.embed(text.trim())
            });
          } else if (runtime.createMemory) {
            // Use the correct ElizaOS runtime API - createMemory(memory, tableName, unique)
            const memoryId = await runtime.createMemory(
              {
                entityId: messageUserId,
                agentId: runtime.agentId,
                roomId: messageRoomId,
                content: {
                  text: text.trim(),
            ...
            // Create the memory object with the returned ID
            messageMemory = {
              id: memoryId,
              entityId: messageUserId,
              agentId: runtime.agentId,
            ...
            // Fallback - use basic memory structure
            messageMemory = {
              id: `msg-${Date.now()}`,
              entityId: messageUserId,
              content: {
          ...
          console.error('[GAME-API] Memory creation failed:', memoryError);
        ...
        console.log('[GAME-API] Created message memory:', messageMemory.id || 'fallback');

        // Process the message through the agent
        try {
          if (runtime.processMessage) {
            await runtime.processMessage(messageMemory);
            console.log('[GAME-API] Message processed successfully');
          } else {
            console.log('[GAME-API] No processMessage method available - message stored only');
          }
        } catch (processingError) {
          console.error('[GAME-API] Error processing message:', processingError);
          // Continue anyway - the message was saved
        }

        // Return success
        res.json(successResponse({
          messageId: messageMemory.id,
          message: 'Message ingested successfully',
          roomId: messageRoomId
        }));
      } catch (error) {
        console.error('[GAME-API] Chat message ingestion failed:', error);
        res.status(500).json(errorResponse('INGEST_FAILED', 'Failed to ingest message', error.message));
      }
    }
  },
    ...
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        ...
        if (!capabilityMappings[capability]) {
          return res.status(400).json(errorResponse('UNKNOWN_CAPABILITY', `Unknown capability: ${capability}`));
        }
        ...
        if (capability === 'autonomy') {
          ...
          if (autonomyService) {
            try {
              ...
              console.warn(`[API] Failed to ${newState ? 'enable' : 'disable'} autonomy service:`, autonomyError);
            }
          }
        }
        ...
        if (['camera', 'microphone', 'speakers', 'screen'].includes(capability)) {
          ...
          if (visionService && typeof (visionService as any).updateConfig === 'function') {
            try {
              ...
              console.warn('[API] Failed to update vision service config:', visionError);
            }
          }
        }
        ...
        if (capability === 'camera' || capability === 'microphone' || capability === 'speakers' || capability === 'screen') {
          ...
          serviceAvailable = !!(runtime.getService('AUTONOMY') || runtime.getService('autonomy'));
        }
        ...
        console.error('[API] Capability toggle error:', error);
        res.status(500).json(errorResponse('CAPABILITY_TOGGLE_ERROR', 'Failed to toggle capability', error.message));
      }
    }
  },
  ...
  // Knowledge Files API (expected by frontend)
  {
    ...
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        ...
        console.error('[GAME-API] Error fetching knowledge files:', error);

...
export class AgentServer {
        ...
        try {
          ...
          switch (message.type) {
            case 'connect':
              ...
              logger.info(`[WebSocket ${clientId}] Client connecting - Agent: ${agent_id}, Channel: ${channel_id}, Type: ${client_type}`);
            ...
            case 'message':
              ...
              logger.info(`[WebSocket ${clientId}] Processing message from ${author}: ${content?.substring(0, 50)}...`);
            ...
            case 'update_log_filters':
              ...
              if (existingFilters !== undefined) {
                ...
                logger.warn(`[WebSocket ${clientId}] Cannot update filters: not subscribed to log stream`);
                ws.send(JSON.stringify({
                  type: 'log_filters_updated',
                  success: false,
          ...
          logger.error(`[WebSocket ${clientId}] Error processing message:`, error);
          ws.send(JSON.stringify({
            type: 'error',
   ...
   */
  private async processWebSocketMessage(messageData: {
    channelId: string;
    agentId: string;
    content: string;
    author: string;
   ...
   */
  public broadcastToWebSocketClients(message: any, _channelId?: string, _excludeClientId?: string): void {
      ...
      this.webSocketServer.clients.forEach((client) => {
        if (client.readyState === 1) { // WebSocket.OPEN
          ...
          if (filters.level && filters.level !== 'all') {
            const numericLevel = typeof filters.level === 'string'
  ...
  async findOrCreateCentralDmChannel(
    ...
    const channel = await this.serverDatabase.findOrCreateDmChannel(user1Id, user2Id, messageServerId);
    return {
      ...channel,
      id: channel.id as UUID,
      type: channel.type as ChannelType
  ...
  async createMessage(
    ...
    if (channel) {
      ...
      if (centralMessage.metadata?.source === 'agent_response' || centralMessage.sourceType?.includes('agent')) {
        this.broadcastToWebSocketClients({
          type: 'agent_message',
          id: centralMessage.id,
          content: centralMessage.content,
...
export {
  hasValidRemoteUrls, jsonToCharacter,
  loadCharacter, loadCharacters, loadCharactersFromUrl, loadCharacterTryPath, tryLoadFile
} from './loader';

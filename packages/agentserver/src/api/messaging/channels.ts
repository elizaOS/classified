...
export function createChannelsRouter(
  ...
  (router as any).post(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        // Emit to Websocket for real-time display in all connected GUIs
        if (serverInstance.websocket) {
          serverInstance.websocket.to(channelIdParam).emit('messageBroadcast', {
            senderId: author_id,
  ...
  (router as any).post('/central-channels', async (req: express.Request, res: express.Response) => {
    ...
    // Accept both serverId (camelCase) and server_id (snake_case)
    const serverIdValue = serverId || server_id;

    // Special handling for default server ID "0"
    const isValidServerId = serverIdValue === DEFAULT_SERVER_ID || validateUuid(serverIdValue);
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        error:
          'Invalid payload. Required: name, serverId (UUID or "0"), participantCentralUserIds (array of UUIDs). Optional: type, metadata.',
      });
    }

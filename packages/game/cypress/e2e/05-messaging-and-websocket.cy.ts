...
describe('Messaging and WebSocket', () => {
  ...
  describe('Message Sending', () => {
    it('should send message to agent via API', () => {
      ...
      cy.request('POST', `${BACKEND_URL}/api/agents/${DEFAULT_AGENT_ID}/message`, testMessage)
        .then((response) => {
    ...
    it('should handle multiple messages in sequence', () => {
      ...
      cy.request('GET', `${BACKEND_URL}/api/memories?roomId=${roomId}&count=10`)
        .then((response) => {
  ...
  describe('Message Broadcasting', () => {
    ...
    it('should handle room-specific messaging', () => {
      ...
      cy.window().then((win: any) => {
        ...
        // Listen for messages
        client.socket.on('message', (data) => {
          if (data.roomId === room1) room1Messages.push(data);
          if (data.roomId === room2) room2Messages.push(data);
        });
        ...
        cy.wait(3000).then(() => {
          ...
          cy.log(`✅ Room isolation working: Room1=${room1Messages.length}, Room2=${room2Messages.length}`);
        });
      });
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle WebSocket disconnection', () => {
      cy.window().then((win: any) => {
        const client = win.elizaClient;
        const originalState = client.socket.connected;
  ...
  describe('Message Persistence', () => {
    ...
    it('should maintain message order', () => {
      ...
      cy.request('GET', `${BACKEND_URL}/api/memories?roomId=${roomId}&count=20`)
        .then((response) => {
  ...
  it('should verify complete messaging functionality', () => {
    ...
    cy.window().then((win: any) => {
      ...
      if (wsConnected) {
        ...
        return cy.request('POST', `${BACKEND_URL}/api/agents/15aec527-fb92-0792-91b6-becd4fac5050/message`, {

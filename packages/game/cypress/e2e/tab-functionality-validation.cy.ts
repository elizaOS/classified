          ...
          cy.get(selector).first().then(($el) => {
            if ($el.is('button') || $el.attr('role') === 'tab') {
              ...
              cy.get('body').then(($body) => {
                ...
                expect(text.includes('monologue') || text.includes('thinking') || text.includes('Agent')).to.be.true;
              });
            }
          });
        }
        ...
        if ($body.find(selector).length > 0) {
          ...
          cy.get(selector).first().then(($el) => {
            if ($el.is('button') || $el.attr('role') === 'tab') {
              ...
              cy.get('body').then(($body) => {
                const text = $body.text();
                expect(text.includes('files') || text.includes('upload') || text.includes('knowledge')).to.be.true;
              });
            }
          });
        }

...
describe('Backend API', () => {
  ...
  describe('Memory System API', () => {
    ...
    it('should retrieve agent-specific memories', () => {
      cy.request('GET', `${BACKEND_URL}/api/agents/${DEFAULT_AGENT_ID}/memories?count=5`)
        .then((response) => {
          expect(response.status).to.eq(200);
          expect(response.body.success).to.be.true;
          expect(response.body.data).to.be.an('array');
    ...
    it('should handle memory pagination', () => {
      cy.request('GET', `${BACKEND_URL}/api/memories?roomId=${testRoomId}&count=5&page=1`)
        .then((response) => {
          expect(response.status).to.eq(200);
          expect(response.body.success).to.be.true;
          expect(response.body.data).to.be.an('array');
  ...
  describe('Concurrent Request Handling', () => {
    it('should handle multiple concurrent health checks', () => {
      ...
      for (let i = 0; i < 10; i++) {
        requests.push(
          cy.request('GET', `${BACKEND_URL}/api/server/health`)
        );
      }
    ...
    it('should handle concurrent configuration reads', () => {
      ...
      cy.wrap(Promise.all(requests)).then((responses: any[]) => {
        ...
        responses.forEach(response => {
          ...
          expect(response.body.data.configurations.environment.MODEL_PROVIDER)
            .to.eq(firstResponse.body.data.configurations.environment.MODEL_PROVIDER);
        });
      });
    });
  });

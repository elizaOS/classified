...
describe('ACTUAL UI TABS TEST - Critical Verification', () => {
  ...
  it('should test if goals tab actually shows data from backend', () => {
    cy.log('🔍 CRITICAL TEST: Does Goals tab actually display backend data?');
    
    // First verify backend has goals data
    cy.request('GET', 'http://localhost:7777/api/goals').then((response) => {
      ...
      // Now check if Goals tab exists and shows this data
      cy.get('body').then(($body) => {
        if ($body.find('[data-testid="goals-tab"]').length > 0) {
          ...
          if ($body.find('[data-testid="goals-content"]').length > 0) {
            ...
            // Check if it shows goals or empty state
            cy.get('[data-testid="goals-content"]').then(($content) => {
              const contentText = $content.text();
              if (contentText.includes('goal') || contentText.includes('GOAL') || $content.find('.status-item').length > 0) {
                cy.log('✅ Goals content appears to show goals data');
              } else {
                cy.log('❌ Goals content does not show expected goals data');
    ...
    cy.screenshot('goals-tab-data-test');
  });

  it('should test if todos tab actually shows data from backend', () => {
    cy.log('🔍 CRITICAL TEST: Does Todos tab actually display backend data?');
    
    // First verify backend has todos data
    cy.request('GET', 'http://localhost:7777/api/todos').then((response) => {
      expect(response.status).to.eq(200);
      cy.log('Backend todos API accessible');
      
      // Now check if Todos tab exists and shows this data
      cy.get('body').then(($body) => {
        if ($body.find('[data-testid="todos-tab"]').length > 0) {
          ...
          if ($body.find('[data-testid="todos-content"]').length > 0) {
            ...
            // Check if it shows todos or empty state
            cy.get('[data-testid="todos-content"]').then(($content) => {
              const contentText = $content.text();
              if (contentText.includes('todo') || contentText.includes('TODO') || contentText.includes('task') || contentText.includes('TASK') || $content.find('.status-item').length > 0) {
                cy.log('✅ Todos content appears to show todos data');
              } else {
                cy.log('❌ Todos content does not show expected todos data');
    ...
    cy.screenshot('todos-tab-data-test');
  });

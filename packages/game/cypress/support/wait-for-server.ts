  ...
  const waitForServer = (retries = maxRetries) => {
    ...
    return cy.request({
      ...
      if (response.status === 200) {
        ...
        cy.log(`⏳ Server not ready yet (status: ${response.status}), retrying... (${maxRetries - retries + 1}/${maxRetries})`);
        cy.wait(5000);
        return waitForServer(retries - 1);
      }
    }).catch(() => {
      cy.log(`⏳ Server connection failed, retrying... (${maxRetries - retries + 1}/${maxRetries})`);
      cy.wait(5000);
      return waitForServer(retries - 1);
    });
  };

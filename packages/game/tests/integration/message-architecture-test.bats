...
@test "Message Architecture Demo with 'hello how are you?'" {
    ...
    local channel_exists=$(PGPASSWORD=eliza psql -h localhost -p 7771 -U eliza -d eliza -t -c "
        SELECT COUNT(*) FROM channels WHERE id = 'game-ui-channel';
    " 2>/dev/null | xargs)
    
    local entity_exists=$(PGPASSWORD=eliza psql -h localhost -p 7771 -U eliza -d eliza -t -c "

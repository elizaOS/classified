...
@test "Message Architecture Demo with 'hello how are you?'" {
    ...
    for i in {1..30}; do
        if PGPASSWORD=eliza psql -h localhost -p 7654 -U eliza -d eliza -c "SELECT 1;" >/dev/null 2>&1; then
            postgres_ready=true
            break
        fi
        sleep 2
    ...
    PGPASSWORD=eliza psql -h localhost -p 7654 -U eliza -d eliza -c "
    ...
    local channel_exists=$(PGPASSWORD=eliza psql -h localhost -p 7654 -U eliza -d eliza -t -c "
        SELECT COUNT(*) FROM channels WHERE id = 'game-ui-channel';
    " 2>/dev/null | xargs)
    
    local entity_exists=$(PGPASSWORD=eliza psql -h localhost -p 7654 -U eliza -d eliza -t -c "

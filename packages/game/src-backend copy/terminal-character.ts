...
You should be autonomous when enabled, setting your own goals and tasks, but also responsive to the Admin when they communicate with you.`,
    ...
    // Agent configuration with proper structure
    settings: {
        // Core model settings
        model: 'gpt-4o-mini',
        temperature: 0.7,
    ...
    // Explicit secrets section for runtime access
    secrets: {
        OPENAI_API_KEY: process.env.OPENAI_API_KEY || '',
        ANTHROPIC_API_KEY: process.env.ANTHROPIC_API_KEY || ''
    },
    ...
    plugins: [
        ...
        '@elizaos/plugin-openai',     // OpenAI provider for embeddings and LLM
        '@elizaos/plugin-sql',        // Database for memories/state
        '@elizaos/plugin-knowledge',  // Knowledge base and file management (after openai)

...
export default function BootSequence({ onComplete, onConnectionTest }: BootSequenceProps) {
  ...
  const [currentPhase, setCurrentPhase] = useState<'boot' | 'connection' | 'testing' | 'setup'>(
    'boot'
  );
  ...
  const testConnection = useCallback(async () => {
    ...
    try {
      ...
      if (configResponse.ok) {
        ...
        needsSetup =
          (!env?.OPENAI_API_KEY || env.OPENAI_API_KEY === 'NOT_SET') &&
      ...
      const isConnected = onConnectionTest
        ? await onConnectionTest()
        : await defaultConnectionTest();
  ...
  const handleSetupComplete = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setMessages((prev) => [...prev, '✗ Failed to save configuration. Please try again.']);
      }
    } catch (error) {
      ...
      setMessages((prev) => [
        ...prev,
        '✗ Error saving configuration. Please check your connection.',
      ]);
    }
  };
          ...
          <div className="setup-form">
            ...
            <div className="setup-info">
              <p className="info-text">
                Your API keys are stored locally and never transmitted to third parties.
              </p>
              <p className="info-text">
  ...
  return (
    <div className="boot-screen">
      <div className="boot-content">
        ...
        {showCursor && (
          <span className="cursor-blink" style={{ color: getStatusColor(connectionStatus) }}>
            █
          </span>
        )}
        {!showCursor && (

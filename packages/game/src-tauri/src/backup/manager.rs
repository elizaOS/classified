...
impl BackupManager {
    ...
    pub async fn create_backup(&self, backup_type: BackupType, notes: Option<String>) -> BackupResult<Backup> {
    ...
    async fn backup_postgres(&self, backup_dir: &Path) -> BackupResult<BackupComponent> {
        ...
        if !dump_output.status.success() {
            ...
            return Err(BackupError::DatabaseBackup(format!("pg_dump failed: {}", stderr)));
        }
    ...
    async fn backup_agent_volumes(&self, backup_dir: &Path) -> BackupResult<Vec<BackupComponent>> {
        ...
        for (volume_name, component_type) in volumes {
            match self.backup_volume(volume_name, component_type, backup_dir).await {
    ...
    async fn backup_volume(&self, volume_name: &str, component_type: ComponentType, backup_dir: &Path) -> BackupResult<BackupComponent> {
        ...
        if !output.status.success() {
            return Err(BackupError::AgentStateBackup(format!("Failed to create backup container")));
        }
        ...
        if !output.status.success() {
            // Clean up
            let _ = self.run_command(vec!["podman", "rm", "-f", &temp_container]).await;
            return Err(BackupError::AgentStateBackup(format!("Failed to create volume backup")));
        }
        ...
        if !output.status.success() {
            return Err(BackupError::AgentStateBackup(format!("Failed to copy backup file")));
        }
    ...
    pub async fn restore_backup(&self, backup_id: &str, options: RestoreOptions) -> BackupResult<()> {
        ...
        for component in &backup.components {
            match component.component_type {
                ...
                ComponentType::AgentState if options.restore_agent_state => {
                    self.restore_volume("eliza-agent-data", &component.path).await?;
                }
                ComponentType::AgentKnowledge if options.restore_knowledge => {
                    self.restore_volume("eliza-agent-knowledge", &component.path).await?;
                }
                ComponentType::AgentLogs if options.restore_logs => {
                    self.restore_volume("eliza-agent-logs", &component.path).await?;
                }
                _ => {
        ...
        let _ = self.run_command(vec!["podman", "start", "eliza-postgres"]).await;
        let _ = self.run_command(vec!["podman", "start", "eliza-ollama"]).await;
        let _ = self.run_command(vec!["podman", "start", "eliza-agent"]).await;
    ...
    async fn restore_postgres(&self, backup_file: &Path) -> BackupResult<()> {
        ...
        let output = self.run_command(decompress_cmd).await
            .map_err(|e| BackupError::RestoreFailed(format!("Database restore failed: {}", e)))?;
        
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(BackupError::RestoreFailed(format!("psql failed: {}", stderr)));
        }
    ...
    async fn restore_volume(&self, volume_name: &str, backup_file: &Path) -> BackupResult<()> {
        ...
        let restore_cmd = vec![
            "podman", "run", "--rm",
            "-v", &volume_mount_restore,
            "-v", &backup_mount,
            "alpine:latest",
            "tar", "-xzf", "/backup.tar.gz", "-C", "/data",
        ];
        
        let output = self.run_command(restore_cmd).await
            .map_err(|e| BackupError::RestoreFailed(format!("Volume restore failed: {}", e)))?;
        
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(BackupError::RestoreFailed(format!("tar failed: {}", stderr)));
        }
            ...
            if path.is_dir() {
                ...
                if manifest_path.exists() {
                    match fs::read_to_string(&manifest_path) {
                        Ok(data) => {
                            match serde_json::from_str::<Backup>(&data) {
    ...
    async fn get_backup_metadata(&self, notes: Option<String>) -> BackupResult<BackupMetadata> {
        ...
        let agent_info_cmd = vec![
            "podman", "exec", "eliza-agent",
            "cat", "/app/agent.json",
        ];
    ...
    pub async fn create_shutdown_backup(&self) -> BackupResult<()> {
        ...
        match self.create_backup(BackupType::Shutdown, Some("Automatic shutdown backup".to_string())).await {
            Ok(backup) => {

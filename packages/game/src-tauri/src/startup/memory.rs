...
impl MemoryConfig {
    ...
    pub fn calculate() -> Self {
        ...
        info!("System total memory: {:.1} GB ({} MB)", total_memory_gb, total_memory_mb);
        ...
        let podman_machine_memory_mb = match total_memory_mb {
            ...
            0..=8192 => {
                if total_memory_mb >= 7680 { // At least 7.5GB total
                    6656
                } else {
    ...
    pub async fn get_current_podman_memory() -> BackendResult<Option<u64>> {
        let output = tokio::process::Command::new("podman")
            ...
            .output()
            .await
            .map_err(|e| BackendError::Container(format!("Failed to inspect Podman machine: {}", e)))?;
    ...
    pub async fn apply_to_podman_machine(&self) -> BackendResult<()> {
        ...
        info!("Configuring Podman machine with {} MB memory", self.podman_machine_memory_mb);
        ...
        let set_output = tokio::process::Command::new("podman")
            ...
            .output()
            .await
            .map_err(|e| BackendError::Container(format!("Failed to set Podman machine memory: {}", e)))?;

...
use crate::backend::{ContainerRuntimeType, BackendResult, BackendError};
...
impl StartupManager {
    ...
    pub async fn start_initialization(&mut self, resource_dir: std::path::PathBuf) -> BackendResult<()> {
        ...
        if let Ok(true) = self.check_existing_server().await {
            ...
            self.update_status(StartupStage::MessageServerReady, 90, 
                "ElizaOS server detected", 
                "Using existing server instance").await;
            
            // Skip directly to ready
            self.update_status(StartupStage::Ready, 100, 
                "Ready to use ELIZA", 
                "Connected to existing ElizaOS server").await;
            
            return Ok(());
        }
        
        // If no existing server, we must start containers - ensure Podman is ready
        self.update_status(StartupStage::DetectingRuntime, 5, 
            "Checking Podman machine status...", 
            "Ensuring container runtime is ready").await;
            
        if let Err(e) = self.ensure_podman_ready().await {
            error!("Podman setup failed: {}", e);
            self.update_status(StartupStage::Error, 0, 
                "Podman setup failed", 
                &format!("Please run 'podman machine start' and try again. Error: {}", e)).await;
            return Err(e);
        }
        
        // Stage 1: Runtime Detection (only if no existing server)
        self.update_status(StartupStage::DetectingRuntime, 10, 
            "Detecting container runtime...", 
            "Checking for Podman, Docker, or downloading runtime").await;
        ...
        match ContainerManager::new_with_runtime_manager(
            ...
            Ok(manager) => {
                ...
                self.update_status(StartupStage::RuntimeDetected, 20, 
                    "Container runtime detected successfully", 
                    "Ready to configure containers").await;
            }
            Err(e) => {
        ...
        self.update_status(StartupStage::PromptingConfig, 25, 
            "Using default configuration...", 
            "Applying default AI provider and container settings").await;
    ...
    pub async fn handle_user_config(&mut self, config: UserConfig) -> BackendResult<()> {
        ...
        self.update_status(StartupStage::ConfigReceived, 30, 
            "Configuration received", 
            "Starting container setup...").await;

        // Stage 3: Ensure agent image is available before setting up containers
        self.update_status(StartupStage::InitializingContainers, 35, 
            "Ensuring agent image is available...", 
            "Building container image if needed").await;
    ...
    async fn setup_containers(&mut self) -> BackendResult<()> {
        let container_manager = self.container_manager.as_ref()
            .ok_or_else(|| BackendError::Container("Container manager not initialized".to_string()))?;

        let config = self.config.as_ref()
            .ok_or_else(|| BackendError::Container("User config not available".to_string()))?;

        self.update_status(StartupStage::InitializingContainers, 45, 
            "Setting up containers...", 
            "Preparing container environment").await;
        ...
        if let Err(e) = self.create_container_network(container_manager).await {
            warn!("Failed to create container network: {}. Containers may have connectivity issues.", e);
        }

        // Start all containers in parallel for faster startup
        self.update_status(StartupStage::StartingDatabase, 55, 
            "Starting all containers in parallel...", 
            "PostgreSQL, Ollama, and Agent starting simultaneously").await;
        ...
        let ollama_task = if matches!(config.ai_provider, AiProvider::Ollama) || config.use_local_ollama {
            if ollama_required {
        ...
        if let Some(task) = postgres_task {
            ...
            match task.await {
                Ok(("postgres", Ok(_status))) => {
                    ...
                    match self.verify_container_health(container_manager, "postgres", "eliza-postgres").await {
                        Ok(true) => {
                            ...
                            self.update_status(StartupStage::StartingDatabase, progress, 
                                "PostgreSQL container ready", 
                                &format!("{}/{} containers started", completed_count, total_count)).await;
                        }
                        Ok(false) => {
                            let error = BackendError::Container("PostgreSQL container started but failed health check".to_string());
                ...
                Ok(("postgres", Err(e))) => {
                    failed_containers.insert("postgres", e);
                    error!("❌ Failed to start PostgreSQL container: {}", failed_containers["postgres"]);
                    self.update_container_status("postgres", "failed").await;
                }
                Err(e) => {
                    failed_containers.insert("postgres", BackendError::Container(format!("Task error: {}", e)));
        ...
        if let Some(task) = ollama_task {
            ...
            match task.await {
                ...
                Ok(("ollama", Err(e))) => {
                    failed_containers.insert("ollama", e);
                    if ollama_required {
                        error!("❌ Failed to start required Ollama container: {}", failed_containers["ollama"]);
                    } else {
                        warn!("⚠️ Failed to start optional Ollama container: {}", failed_containers["ollama"]);
                    }
                    self.update_container_status("ollama", "failed").await;
                }
                Err(e) => {
                    failed_containers.insert("ollama", BackendError::Container(format!("Task error: {}", e)));
        ...
        for required_container in &required_containers {
            if let Some(error) = failed_containers.get(required_container) {
                self.update_status(StartupStage::Error, 0, 
                    ...
                    &format!("Error: {}. This container is required for game functionality.", error)).await;
                return Err(BackendError::Container(format!("Failed to start required {} container: {}", required_container, error)));
            }
        }

        info!("🎉 All required containers started successfully! ({}/{} total)", completed_count, total_count);

        // Stage 4: Wait for container health checks
        self.update_status(StartupStage::WaitingForHealth, 85, 
            "Waiting for containers to be healthy...", 
            "Performing health checks").await;

        self.wait_for_container_health().await?;

        // Stage 5: All containers ready
        self.update_status(StartupStage::ContainersReady, 95, 
            "All containers are healthy", 
            "Finalizing initialization").await;

        // Stage 6: Everything ready!
        self.update_status(StartupStage::Ready, 100, 
            "ELIZA Game ready!", 
            "All systems initialized - ready for chat").await;
    ...
    async fn wait_for_container_health(&self) -> BackendResult<()> {
        ...
        for i in 0..max_wait {
            ...
            let all_healthy = statuses.iter().all(|status| {
                matches!(status.health, crate::backend::HealthStatus::Healthy)
            });
            ...
            if i % 5 == 0 {
                info!("⏳ Waiting for containers to be healthy... ({}/{})", i, max_wait);
            }

            tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
        }

        Err(BackendError::Container("Containers failed to become healthy within timeout".to_string()))
    }
    ...
    async fn update_container_status(&self, container_name: &str, status: &str) {
        ...
        startup_status.container_statuses.insert(container_name.to_string(), status.to_string());
    ...
    async fn create_container_network(&self, container_manager: &Arc<ContainerManager>) -> BackendResult<()> {
    ...
    async fn ensure_podman_ready(&self) -> BackendResult<()> {
        ...
        let output = tokio::process::Command::new("podman")
            ...
            .output()
            .await
            .map_err(|e| BackendError::Container(format!("Failed to check Podman machine: {}", e)))?;
        ...
        if !output_str.contains("Currently running") {
            ...
            let start_output = tokio::process::Command::new("podman")
                ...
                .output()
                .await
                .map_err(|e| BackendError::Container(format!("Failed to start Podman machine: {}", e)))?;
                
            if !start_output.status.success() {
                let error_str = String::from_utf8_lossy(&start_output.stderr);
                return Err(BackendError::Container(format!("Podman machine start failed: {}", error_str)));
            }
        ...
        let test_output = tokio::process::Command::new("podman")
            ...
            .output()
            .await
            .map_err(|e| BackendError::Container(format!("Failed to test Podman connection: {}", e)))?;
            
        if !test_output.status.success() {
            let error_str = String::from_utf8_lossy(&test_output.stderr);
            return Err(BackendError::Container(format!("Podman connection test failed: {}", error_str)));
        }
    ...
    async fn ensure_agentserver_image(&self) -> BackendResult<()> {
        ...
        let current_dir = std::env::current_dir()
            .map_err(|e| BackendError::Container(format!("Failed to get current directory: {}", e)))?;
        ...
        if !binary_output.status.success() {
            ...
            return Err(BackendError::Container(format!("Binary build failed: {}", error_str)));
        }
        
        info!("🐳 Building Docker image...");
        let docker_output = tokio::process::Command::new("podman")
            .args(&["build", "-f", "Dockerfile.standalone", "-t", "eliza-agent-server:latest", "."])
            .current_dir(&agentserver_dir)
            .output()
            .await
        ...
        if !docker_output.status.success() {
            ...
            return Err(BackendError::Container(format!("Image build failed: {}", error_str)));
        }
    ...
    async fn verify_container_health(&self, container_manager: &Arc<ContainerManager>, service_name: &str, container_name: &str) -> BackendResult<bool> {
        ...
        match container_manager.get_runtime_container_status(container_name).await {
            Ok(status) => {
                match status.state {
                    ...
                    crate::backend::ContainerState::Starting => {
                        ...
                        while attempts < 10 {
                            ...
                            match container_manager.get_runtime_container_status(container_name).await {
                                Ok(status) if status.state == crate::backend::ContainerState::Running => {
                                    info!("✅ {} container finished starting", service_name);
                                    return Box::pin(self.verify_container_health(container_manager, service_name, container_name)).await;
                                }
                                Ok(status) if status.state == crate::backend::ContainerState::Stopped => {
                                    error!("❌ {} container stopped unexpectedly", service_name);
                                    return Ok(false);
                                }
                                _ => {
                                    info!("⏳ {} container still starting... (attempt {}/10)", service_name, attempts);
                                }
                            }
                        }
                        
                        error!("❌ {} container failed to start within timeout", service_name);
                        Ok(false)
                    }
                    _ => {
                        error!("❌ {} container is in unexpected state: {:?}", service_name, status.state);
                        Ok(false)
                    }
                }
            }
    ...
    async fn test_postgres_connection(&self) -> BackendResult<bool> {
        ...
        let test_output = tokio::process::Command::new("podman")
            .args(&["exec", "eliza-postgres", "pg_isready", "-U", "eliza", "-d", "eliza_game"])
            .output()
            .await
            .map_err(|e| BackendError::Container(format!("Failed to test PostgreSQL connection: {}", e)))?;
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    warn!("⚠️ Agent API health check returned status: {}", response.status());
                    Ok(false)
                }
            }
            Err(e) => {
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    warn!("⚠️ Ollama API health check returned status: {}", response.status());
                    Ok(false)
                }
            }
            Err(e) => {

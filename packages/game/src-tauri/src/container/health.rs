use crate::backend::{BackendError, BackendResult, HealthCheckConfig, ContainerState, HealthStatus};
    ...
    pub async fn start_monitoring(
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                debug!("Performing health check for container: {}", container_name_clone);
                
                match perform_health_check(&container_name_clone, &command, timeout).await {
                    Ok(is_healthy) => {
                        consecutive_failures = 0;
                        update_container_health(&containers_clone, &container_name_clone, is_healthy);
                    ...
                    Err(e) => {
                        ...
                        warn!("Health check error for container {}: {} (failure {}/{})", 
                              container_name_clone, e, consecutive_failures, max_retries);
                        
                        if consecutive_failures >= max_retries {
                            error!("Container {} marked as unhealthy after {} consecutive failures", 
                                   container_name_clone, consecutive_failures);
                            update_container_health(&containers_clone, &container_name_clone, false);
                        }
                    }
                }
            }
        ...
        self.checks.insert(container_name.clone(), HealthCheckTask { handle });
        info!("Started health monitoring for container: {}", container_name);
    }

    pub async fn stop_monitoring(&self, container_name: &str) {
        if let Some((_, task)) = self.checks.remove(container_name) {
            task.handle.abort();
            info!("Stopped health monitoring for container: {}", container_name);
        }
    }

    pub async fn stop_all_monitoring(&self) {
        let container_names: Vec<String> = self.checks.iter().map(|entry| entry.key().clone()).collect();
...
async fn perform_health_check(
    ...
    if command.is_empty() {
        return Err(BackendError::Container("Empty health check command".to_string()));
    }
    ...
    match tokio::time::timeout(timeout, cmd.output()).await {
        ...
        Ok(Err(e)) => Err(BackendError::Container(format!("Health check execution failed: {e}"))),
        Err(_) => Err(BackendError::Container("Health check timed out".to_string())),
    }
}

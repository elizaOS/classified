...
impl ModelManager {
    ...
    async fn download_model(ollama_url: &str, model_name: &str) -> BackendResult<()> {
        ...
        if response.status().is_success() {
            ...
            Err(BackendError::Container(
                format!("Failed to download model {}: HTTP {}", model_name, response.status())
            ))
        }
    }
    ...
    async fn test_text_generation(ollama_url: &str, model: &str) -> BackendResult<()> {
        ...
        if response.status().is_success() {
            ...
            Err(BackendError::Container(
                format!("Text generation test failed: HTTP {}", response.status())
            ))
        }
    }
    ...
    async fn test_embedding_generation(ollama_url: &str, model: &str) -> BackendResult<()> {
        ...
        if response.status().is_success() {
            ...
            Err(BackendError::Container(
                format!("Embedding test failed: HTTP {}", response.status())
            ))
        }
    }
}
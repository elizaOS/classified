...
impl ResourceChecker {
    ...
    pub async fn check_resources(&mut self, requirements: &ResourceRequirements) -> BackendResult<ResourceCheckResult> {
        ...
        let mut result = ResourceCheckResult {
            ...
            disk_check: self.check_disk_space(requirements.min_disk_gb + (requirements.estimated_download_mb / 1024)),
    ...
    fn check_memory(&self, required_mb: u64) -> ResourceStatus {
        ...
        ResourceStatus {
            ...
            message: if available_mb >= required_mb {
                format!("Sufficient memory available: {:.1}GB", available_mb as f64 / 1024.0)
            } else {
                format!(
                                ...
                                .await
                            {
                                ...
                                for task_line in tasklist_str.lines().skip(3) {
                                    if !task_line.is_empty() {
                                        let task_parts: Vec<&str> = task_line.split_whitespace().collect();

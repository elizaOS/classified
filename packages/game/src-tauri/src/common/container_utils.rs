use crate::backend::{BackendResult, BackendError};
    ...
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    {
        ...
        if output.status.success() && !output.stdout.is_empty() {
            ...
            if !pids.is_empty() && !pids[0].is_empty() {
                info!("🚫 Found {} process(es) using port {}: {:?}", pids.len(), port, pids);
                        ...
                        match kill_result {
                            Ok(kill_output) => {
                                if kill_output.status.success() {
                                    ...
                                    warn!("⚠️ SIGTERM failed for PID {}, trying SIGKILL...", pid.trim());
                                    let force_kill = Command::new("kill")
                                        .args(["-KILL", pid.trim()])
                                        .output();

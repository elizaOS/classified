...
pub async fn run_all_tests(app_handle: AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    app_handle.emit("startup-status", StartupStatus {
    ...
    // Report results
    let (passed, failed): (Vec<_>, Vec<_>) = all_results.iter().enumerate().partition(|(_, result)| result.is_ok());
    
    info!("✅ Test Suite Complete: {} passed, {} failed", passed.len(), failed.len());

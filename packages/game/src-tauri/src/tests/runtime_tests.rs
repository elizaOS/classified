...
async fn test_container_management(app_handle: &AppHandle) -> TestResult {
    ...
    let postgres_running = postgres_status.is_ok() && 
        postgres_status.unwrap().state == ContainerState::Running;
    
    let agent_running = agent_status.is_ok() && 
        agent_status.unwrap().state == ContainerState::Running;
...
async fn test_goals_crud(_app_handle: &AppHandle) -> TestResult {
    ...
    let goals_array = updated_goals["data"]["goals"].as_array()
        .ok_or("Goals not an array")?;
    
    let found = goals_array.iter().any(|g| 
        g["name"].as_str() == Some(&test_goal_name)
    );
...
async fn test_todos_crud(_app_handle: &AppHandle) -> TestResult {
    ...
    let todos_array = updated_todos["data"]["todos"].as_array()
        .ok_or("Todos not an array")?;
    
    // Check for 'task' field instead of 'name' when verifying
    let found = todos_array.iter().any(|t| 
        t["task"].as_str() == Some(&test_todo_name) || 
        t["name"].as_str() == Some(&test_todo_name)
    );
...
async fn test_knowledge_crud(_app_handle: &AppHandle) -> TestResult {
    ...
    if let Some(doc_id) = upload_response["data"]["documentId"].as_str() {
        let delete_response = crate::ipc::commands::delete_knowledge_file(
            doc_id.to_string()
        ).await?;
...
async fn test_autonomy_control(_app_handle: &AppHandle) -> TestResult {
    ...
    let initial_enabled = status_response["data"]["enabled"].as_bool().unwrap_or(false);
...
async fn test_logs_endpoint(_app_handle: &AppHandle) -> TestResult {
    ...
    // Fetch logs
    let logs_response = crate::ipc::commands::fetch_logs(
        Some("all".to_string()),
        Some(10)
    ).await?;
...
async fn test_agent_settings() -> TestResult {
    ...
    let update_response = crate::ipc::commands::update_agent_setting(

...
mod port_fallback_tests {
    use app_lib::{PortConfig, ContainerManager, ContainerRuntimeType};
    ...
    async fn test_container_manager_port_config() {
        ...
        let manager = ContainerManager::new_with_runtime_type(
    ...
    async fn test_port_fallback_when_in_use() {
        ...
        if _listener.is_ok() {
            ...
            println!("Port 5432 was in use, alternative port selected: {}", config.postgres_port);
        }
    }

...
async fn test_health_monitor_creation() {
    ...
    assert_eq!(std::mem::size_of_val(&health_monitor), std::mem::size_of::<app_lib::HealthMonitor>());
}
...
fn test_serialization() {
    use app_lib::{ContainerStatus, ContainerState, HealthStatus};
    ...
    let deserialized: ContainerStatus = serde_json::from_str(&json).expect("Should deserialize from JSON");

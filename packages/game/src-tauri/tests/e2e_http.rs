...
async fn test_http_server_e2e() {
    use app_lib::{HttpServer, BackendConfig, ContainerManager, ContainerRuntimeType};
    ...
    let config = BackendConfig { http_port: 7779, ..Default::default() };
...
async fn test_container_endpoints() {
    ...
    let client = reqwest::Client::new();
    let url = "http://127.0.0.1:7778/api/containers/status";
    
    // This test expects the server to be running on port 7778
    ...
    match result {
        Ok(Ok(response)) => {
            if response.status().is_success() {
                ...
                println!("⚠️  Container status endpoint returned error: {}", response.status());
            }
        }
        Ok(Err(e)) => {
            println!("⚠️  Could not connect to HTTP server (expected if not running): {}", e);
        }
        Err(_) => {
...
async fn test_websocket_info_endpoint() {
    let client = reqwest::Client::new();
    let url = "http://127.0.0.1:7778/api/websocket/info";
    ...
    match result {
        Ok(Ok(response)) => {
            if response.status().is_success() {
                ...
                println!("⚠️  WebSocket info endpoint returned error: {}", response.status());
            }
        }
        Ok(Err(e)) => {
            println!("⚠️  Could not connect to HTTP server (expected if not running): {}", e);

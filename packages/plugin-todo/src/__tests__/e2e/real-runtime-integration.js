import { createMessageMemory, asUUID } from '@elizaos/core';
 ...
 */
export const RealRuntimeIntegrationTestSuite = {
    ...
    tests: [
        {
            ...
            fn: async (runtime) => {
                ...
                if (createdTodo.name !== testData.name) {
                    throw new Error(`Data integrity issue: expected ${testData.name}, got ${createdTodo.name}`);
                }
        ...
        },
        {
            ...
            fn: async (runtime) => {
                ...
                const todoActions = runtime.actions.filter((action) => action.name.includes('TODO') ||
                ...
                const testCallback = async (content, files) => {
                ...
                if (completeAction) {
                    ...
                    if (!completeValid) {
                        console.log('⚠️ COMPLETE_TODO validation failed (may be expected if no incomplete todos)');
                    }
                    else {
        ...
        },
        {
            ...
            fn: async (runtime) => {
                ...
                if (typeof reminderService.checkTasksForReminders !== 'function') {
                ...
                try {
                    ...
                    throw new Error(`Reminder service execution failed: ${error instanceof Error ? error.message : String(error)}`);
                }
                // Clean up
        ...
        },
        {
            ...
            fn: async (runtime) => {
                ...
                try {
                    ...
                    console.log(`⚠️ Message processing had issues: ${error instanceof Error ? error.message : String(error)}`);
                }
        ...
        },
        {
            ...
            fn: async (runtime) => {
                ...
                const workflowCallback = async (content, files) => {
                ...
                const possibleTodo = todos.find((todo) => todo.name.toLowerCase().includes('report') ||
                ...
                if (completeAction) {
                    ...
                    const completeValid = await completeAction.validate(runtime, completeMessage, completeState);
                    if (completeValid) {

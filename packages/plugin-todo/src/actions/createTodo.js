import { createUniqueUuid, formatMessages, logger, ModelType, parseKeyValueXml, } from '@elizaos/core';
 ...
 */
const extractTodoTemplate = (text, messageHistory) => `
 ...
 */
async function extractTodoInfo(runtime, message, state) {
    try {
        ...
        const finalTodo = {
            ...
            taskType: validatedTodo.taskType,
        };
        if (finalTodo.taskType === 'one-off') {
            finalTodo.priority = validatedTodo.priority
                ? parseInt(String(validatedTodo.priority), 10)
                : 3;
            ...
            finalTodo.recurring = (validatedTodo.recurring || 'daily');
        }
        return finalTodo;
    }
    catch (error) {
...
export const createTodoAction = {
    ...
    description: 'Creates a new todo item from user description. Supports daily recurring tasks, one-off tasks with priorities and due dates, and aspirational goals. Returns created todo details including ID, name, and type. Can be chained with LIST_TODOS to show updated list.',
    validate: async (_runtime, _message) => {
        // No validation needed if we create directly - let handler decide
        return true;
    },
    handler: async (runtime, message, stateFromTrigger, options, callback, _responses) => {
        let todo = null;
        try {
            ...
            const state = stateFromTrigger || (await runtime.composeState(message, ['TODOS', 'RECENT_MESSAGES']));
            ...
            if (duplicateTodo) {
                logger.warn(`[createTodoAction] Duplicate task found for name "${todo.name}". ID: ${duplicateTodo.id}`);
                if (callback) {
                    await callback({

import { composePrompt, logger, ModelType, parseKeyValueXml, } from '@elizaos/core';
 ...
 */
async function extractTaskSelection(runtime, message, availableTasks) {
    try {
 ...
 */
async function extractTaskUpdate(runtime, message, task) {
    try {
        ...
        // Parse XML from the text results
        const parsedUpdate = parseKeyValueXml(result);
        ...
        const finalUpdate = { ...parsedUpdate };
        if (finalUpdate.priority) {
            ...
            if (!isNaN(priorityVal) && priorityVal >= 1 && priorityVal <= 4) {
                finalUpdate.priority = priorityVal;
            }
            else {
        ...
        if (finalUpdate.recurring) {
            ...
            if (['daily', 'weekly', 'monthly'].includes(recurringVal)) {
                finalUpdate.recurring = recurringVal;
            }
            else {
 ...
 */
async function applyTaskUpdate(dataService, task, update) {
    // Prepare tags array

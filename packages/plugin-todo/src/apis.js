...
export const routes = [
        ...
        handler: async (req, res, runtime) => {
            try {
                ...
                if (!agentRoomIds || agentRoomIds.length === 0) {
                    logger.debug(`[API /api/todos] Agent ${runtime.agentId} is not a participant in any rooms.`);
                    return res.json([]); // No rooms for this agent
                }
                logger.debug(`[API /api/todos] Agent ${runtime.agentId} is in rooms: ${agentRoomIds.join(', ')}`);
                ...
                if (agentRooms.length === 0) {
                    logger.debug("[API /api/todos] No valid room details found for agent's participated rooms.");
                    return res.json([]);
                }
        ...
        handler: async (_req, res, runtime) => {
          ...
          `;
                ...
                const tags = Array.isArray(result)
                    ...
                    : result.rows // Node-postgres likely returns object with 'rows'
                        ? result.rows.map((row) => row.tag)
                        : [];
    ...
    // API route to update an existing TODO
    {
        ...
        handler: async (req, res, runtime) => {
            try {
                const taskId = req.params.id;
                const updateData = req.body; // Directly use interface from updateTodo.ts
                if (!taskId) {

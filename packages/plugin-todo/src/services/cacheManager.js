...
export class CacheManager {
    ...
    async cacheEntity(entityId, entity, ttl = 5 * 60 * 1000) {
        await this.set(`entity:${entityId}`, entity, ttl);
    }
    async getCachedEntity(entityId) {
        return await this.get(`entity:${entityId}`);
    }
    async cacheUserBehavior(userId, behavior, ttl = 30 * 60 * 1000) {
        await this.set(`behavior:${userId}`, behavior, ttl);
    }
    async getCachedUserBehavior(userId) {
        return await this.get(`behavior:${userId}`);
    }
    async cacheReminderRecommendation(todoId, recommendation, ttl = 10 * 60 * 1000) {
        await this.set(`recommendation:${todoId}`, recommendation, ttl);
    }
    async getCachedReminderRecommendation(todoId) {
        return await this.get(`recommendation:${todoId}`);
    }
    async cacheServiceHealth(serviceName, health, ttl = 30 * 1000) {
    ...
    async getFrequentlyAccessedEntities() {
        // This would typically query the database for active users
        // For now, return empty array
        return [];
    }
    async warmUpEntity(entityId) {
    ...
    async warmUpServiceHealth(serviceName) {

...
export class TodoDataService {
    ...
    async createTodo(data) {
        try {
            ...
            if (data.tags && data.tags.length > 0) {
                await db.insert(todoTagsTable).values(data.tags.map((tag) => ({
                    todoId: todo.id,
                    tag,
                })));
            }
    ...
    async addTags(todoId, tags) {
        try {
            ...
            if (newTags.length > 0) {
                await db.insert(todoTagsTable).values(newTags.map((tag) => ({
                    todoId,
                    tag,
                })));
            }
    ...
    async removeTags(todoId, tags) {
        try {
            ...
            await db
                ...
                .where(and(eq(todoTagsTable.todoId, todoId), or(...tags.map((tag) => eq(todoTagsTable.tag, tag)))));
            return true;
        }
        catch (error) {
    ...
    async getOverdueTodos(filters) {
        try {
            ...
            // Fetch tags
            const todosWithTags = await Promise.all(overdueTodos.map(async (todo) => {
                const tags = await db
                    .select({ tag: todoTagsTable.tag })
                    .from(todoTagsTable)
                    .where(eq(todoTagsTable.todoId, todo.id));
 ...
 */
export function createTodoDataService(runtime) {

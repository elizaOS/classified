...
async function runPluginE2ETests() {
  try {
    ...
    const adapter = new (class extends DatabaseAdapter {
        ...
        return {
          values: (values) => ({
            returning: async () => {
              ...
              logger.debug(`Insert called on table: ${tableName}, typeof: ${typeof table}, keys: ${Object.keys(table || {}).join(', ')}`);
              
              if (tableName === 'goals' || !tableName || tableName === 'unknown') {
                // Handle goals table
                const id = values.id || `goal-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
                const record = { 
                  ...values, 
                  id,
                ...
                if (Array.isArray(values)) {
                  ...
                  for (const tagData of values) {
                    const tagRecord = {
                      id: tagData.id || `tag-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                  ...
                  logger.debug(`Created ${insertedTags.length} tags for goal: ${insertedTags.map(t => t.tag).join(', ')}`);
                  return insertedTags;
                } else {
                  // Single tag insert
                  const tagRecord = {
      ...
      select() {
        ...
        const buildSelectQuery = () => {
          ...
          if (currentTable === 'goals' || !currentTable) {
            ...
            // Apply the actual filters used by GoalDataService.getGoals()
            if (whereFilters) {
              results = results.filter(goal => {
                ...
                if (whereFilters.isCompleted !== undefined && goal.isCompleted !== whereFilters.isCompleted) {
                  return false;
                }
            ...
            // This mimics how the GoalDataService does it
          ...
          logger.debug(`Query: table=${currentTable}, filters=${JSON.stringify(whereFilters)}, results=${results.length}`);
        ...
        const createQueryBuilder = () => ({
          ...
          where: (condition) => {
            ...
            // For now, we'll catch when GoalDataService passes the real filters
            return createQueryBuilder();
          },
              ...
              where: (condition) => ({
                returning: async () => {
                  ...
                  for (const [id, goal] of self.goals.entries()) {
                    ...
                    const shouldUpdate = condition.toString().includes(id) || 
                                       Object.keys(self.goals).length === 1; // If only one goal, update it
      ...
      async createMemory() { return 'mock-memory-id'; }
      async getMemories() { return []; }
      async updateMemory() { return true; }
      async deleteMemory() { return true; }
      async deleteAllMemories() { return true; }
      async searchMemories() { return []; }
      async getCachedEmbeddings() { return []; }
      async log() { return true; }
      async getActorDetails() { return []; }
      async searchMemoriesByEmbedding() { return []; }
      async createEntity() { return 'mock-entity-id'; }
      ...
      async updateEntity() { return true; }
      async createComponent() { return 'mock-component-id'; }
      async getComponents() { return []; }
      async updateComponent() { return true; }
      async deleteComponent() { return true; }
      ...
      async getEntitiesInRoom() { return []; }
      async getEntityByIds() { return []; }
      ...
      async getEntitiesForRoom() { return []; }
      async getLastMessages() { return []; }
      async removeRoom() { return true; }
      ...
      async addEmbeddingToMemory() { return true; }
      
      // Missing agent-related methods
      async getAgents() { return []; }
      async createAgent() { return 'mock-agent-id'; }
      async updateAgent() { return true; }
      async deleteAgent() { return true; }
      async getAgent() { return null; }
      
      // Missing methods for count operations
      async countMemories() { return 0; }
      async countComponents() { return 0; }
      async countEntities() { return 0; }
      async countRooms() { return 0; }
      async countTasks() { return 0; }
      
      // Missing methods for search operations
      async searchEntities() { return []; }
      async searchComponents() { return []; }
      async searchRooms() { return []; }
      async searchTasks() { return []; }
      
      // Missing methods for bulk operations
      async createMemories() { return []; }
      async createEntities() { return []; }
      async createComponents() { return []; }
      async createRooms() { return []; }
      async createTasks() { return []; }
      
      // Missing methods for embedded/vector operations
      async ensureEmbeddingDimension() { return true; }
      async addEmbedding() { return true; }
      async searchEmbeddings() { return []; }
      
      // Missing utility methods
      async removeAllMemories() { return true; }
      async clearCache() { return true; }
      async vacuum() { return true; }
      async analyze() { return true; }
    })();
    ...
    // Create custom GoalDataService that works with our mock database
    const originalCreateService = (await import('./src/services/goalDataService.js')).createGoalDataService;
    ...
    global.createTestGoalDataService = (runtime) => {
      ...
      service.getGoals = async (filters) => {
        ...
        if (filters?.tags && filters.tags.length > 0) {
          ...
          logger.debug(`Goals before tag filtering: ${goalsWithTags.map(g => `${g.name} (${g.tags.join(', ')})`).join(', ')}`);
          
          const filtered = goalsWithTags.filter(goal => 
            filters.tags.some(tag => goal.tags.includes(tag))
          );
          
          logger.debug(`Goals after tag filtering: ${filtered.map(g => `${g.name} (${g.tags.join(', ')})`).join(', ')}`);
          return filtered;
        }

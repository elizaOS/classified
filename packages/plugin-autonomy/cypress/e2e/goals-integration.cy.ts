...
describe('Goals Integration Tests', () => {
  ...
  it('should be able to complete goals via API', () => {
    ...
    cy.request('GET', `/api/goals?agentId=${agentId}`).then((response) => {
      ...
      if (incompleteGoal) {
        // Complete the goal
        cy.request('PUT', `/api/goals/${incompleteGoal.id}/complete?agentId=${agentId}`).then((completeResponse) => {
  ...
  it('should verify goals persist across plugin reloads', () => {
    ...
    cy.request('POST', `/api/goals?agentId=${agentId}`, persistenceTestGoal).then((createResponse) => {
  ...
  after(() => {
    ...
    cy.request('GET', `/api/goals?agentId=${agentId}`).then((response) => {
      const goals = response.body;
      const testGoals = goals.filter((g: any) => 
        g.name.includes('Test Goal') || 
        g.name.includes('Persistence Test')
      );

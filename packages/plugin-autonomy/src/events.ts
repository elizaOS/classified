...
const handleServerSync = async ({
  ...
  try {
    // The ensureConnection API has changed - it now expects a single connection object
    // We need to iterate through entities and rooms to create individual connections
    for (const room of rooms) {
      for (const entity of entities) {
        await runtime.ensureConnection({
          ...
          source,
          name: (entity.metadata?.name || entity.metadata?.username || entity.names?.[0]) as
            | string
            | undefined,
          channelId: room.metadata?.channelId as string | undefined,
          serverId: world.metadata?.serverId as string | undefined,
          type: (room.metadata?.type as ChannelType) || ChannelType.GROUP,

...
export class AutonomyLogger {
  ...
  constructor(runtime: IAgentRuntime, config?: Partial<LoggerConfig>) {
    ...
    this.config = {
      level: LogLevel.INFO,
      enableFileLogging: this.runtime.getSetting('AUTONOMOUS_FILE_LOGGING') === 'true',
      logDirectory:
        this.runtime.getSetting('AUTONOMOUS_LOG_DIR') ||
        path.join(process.cwd(), '.eliza', 'logs', 'autonomy'),

  ...
  tests: [
    ...
    
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const nonAutonomousResult = await adminChatProvider.get(runtime, nonAutonomousMessage, testState);
    ...
    
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        console.log('✓ Action validation in autonomous context:', shouldAccept ? 'accepted' : 'rejected (admin not configured)');
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (typeof currentStatus.enabled !== 'boolean' ||

...
export class ServerDatabaseAdapter {
   ...
   */
  async addParticipantToChannel(channelId: string, entityId: UUID): Promise<void> {
    const now = new Date();
    await this.db
   ...
   */
  async removeParticipantFromChannel(channelId: string, entityId: UUID): Promise<void> {
    await this.db
      .delete(channelParticipantsTable)
      .where(
        sql`${channelParticipantsTable.channelId} = ${channelId} AND ${channelParticipantsTable.entityId} = ${entityId}`
      );
  }
